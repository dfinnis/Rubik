...
#### -- test 18 -- ####
cmd="./Rubik \"U2 R2 B2 D2 R2 L2 U2 B2 D2 R2 L R U2 F2 F' B R2 F D2 U D\""
output=$(eval "$cmd")
incorrect=$(echo "$output" | tail -n 9 | head -n 1 )
time=$(echo "$output" | tail -n 1)
prefix=$(echo "$time" | rev | cut -c-1-2 | rev | cut -c-1-1)
...
if [ "$incorrect" == "Error: Solution Incorrect :(" ]
then
	echo "\x1b[31mcommand line:\t\tERROR\t	$htm\t$time\x1b[0m"
else
	echo "\x1b[32mcommand line:\t\tOK\t	$htm\t$time\x1b[0m"
	((solved+=1))
fi
((count+=1))
